...
## [2 Separation and Abstraction](https://github.com/AndreasHefti/functionalJava/blob/master/src/test/java/com/andreashefti/exercise/_2_Abstraction.java)

Functional programming is all about separation of data-processing and effects of a program
and abstracting imperative control structures by composing functions instead of results of functions or values like in an imperative style.<p>
A pure function has no side-effect(s) and separates the data-processing part of a program from the effect(s) - which generally are the main
purpose of a program - in a functional style.


### 2.1 Separation of data-processing and effects
...
first and then the composed one.<br>
...
The above example of Effect composition prints the following to the output:
...
So the Effect functional interface can be used to separate an effect from a function.<p>
...
Therefore we introduce the Result type:
    ...
    public abstract class Result<T> implements Serializable {
        ...
        private static final class Success<T> extends Result<T> {
            ...
            public T getOrElse( Supplier<T> defaultValue ) { return value; }
            public Result<T> orElse( Supplier<Result<T>> defaultValue ) { return success( value ); }
        ...
        private static final class Empty<T> extends Result<T> {
            ...
            public T get() { throw new IllegalStateException( "get called on empty Result" ); }
            public T getOrElse( T defaultValue ) { return defaultValue; }
            public T getOrElse( Supplier<T> defaultValue ) { return defaultValue.get(); }
            public Result<T> orElse( Supplier<Result<T>> defaultValue ) { return defaultValue.get(); }
        ...
        private static final class Failure<T> extends Result<T> {
            ...
            public Failure( RuntimeException e ) { exception = e; }
            public Failure( Exception e ) { exception = new IllegalStateException( e.getMessage(), e ); }
            public Failure( String message ) { exception = new IllegalStateException( message); }
            
            public boolean isPresent() { return false; }
            public T get() { throw exception; }
            public T getOrElse( final T defaultValue ) { return defaultValue; }
            public T getOrElse( final Supplier<T> defaultValue ) { return defaultValue.get(); }
            public Result<T> orElse( Supplier<Result<T>> defaultValue ) { return defaultValue.get(); }
            public Optional<T> toOptional() { return Optional.empty(); }
        }
...
This Result type can be used as a return value for a method or function to guarantee further processing 
without NullPointerException or the need of explicit error handling.
<p>
...
Now we define a Map with some values:
...
### 2.1 Abstracting control structures
