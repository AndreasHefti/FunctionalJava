...
public class _1_BasicFunctional {
     ...
     *
     * We can also write the compose as a "stand alone" polymorphic higher-order method like that:
     *
     ...
     */
    @Test
    public void higherOrderMethod() {
        Function<Integer, Integer> triple = x -> x * 3;
        Function<Integer, Integer> square = x -> x * x;

        Function<Integer, Integer> f1 = triple.compose( square );
        Function<Integer, Integer> f2 = square.compose( triple );

        Function<Integer, Integer> f3 = triple.andThen( square );
        Function<Integer, Integer> f4 = square.andThen( triple );

        assertTrue( 12 == f1.apply( 2 ) );
        assertTrue( 36 == f2.apply( 2 ) );

        assertTrue( 36 == f3.apply( 2 ) );
        assertTrue( 12 == f4.apply( 2 ) );

        Function<Integer, Integer> f5 = compose( triple, square );
        Function<Integer, Integer> f6 = compose( square, triple );

        assertTrue( 12 == f5.apply( 2 ) );
        assertTrue( 36 == f6.apply( 2 ) );
    }
     ...
     *
     *  Note that this is only true for local variables but not for class variables.
     *
     *      double taxRate = 2;
     *
     *      public void someMethod() {
     ...
     *      taxRate = 3;
     *
     *  NOTE: that Closures can cause problems on refactorings or when functions are passed to other functions as arguments
     *        because the dependencies of Closures are not necessarily clear or easy to see or define.
     */
    @Test
    public void closures() {
        ...
        double taxRate1 = 2;
        double taxRate2 = 2;
        Function<Double, Double> closure1 = price -> price * taxRate1;

        // taxRate1 = 3; // compiler error

        Function<Double, Double> closure2 = closureExample( taxRate2 );
        taxRate2 = 3;

        assertTrue( 4 == closure1.apply( 2d ) );
        assertTrue( 4 == closure2.apply( 2d ) );
        assertTrue( 3 == taxRate2 );
    }

    static Function<Double, Double> closureExample( double taxRate ) {
    ...
    public void functionCurryingAndPartialApplying() {
        ...
        Op3<Integer, Integer, Integer, Integer> addThree = i1 -> i2 -> i3 -> i1 + i2 + i3;
        assertEquals( Integer.valueOf( 10 ), addThree.apply( 2 ).apply( 3 ).apply( 5 ) );

        Function<Integer, Function<Integer, Integer>> addTo2 = addThree.apply( 2 );
        Function<Integer, Integer> addTo5 = addTo2.apply( 3 );

        // function composition with a curried higher order function
        Function<Integer, String> intToString = i -> String.valueOf( i );
        Function<String, Integer> length = s -> s.length();

        assertTrue( 3 == this.<Integer, String, Integer>COMPOSE().apply( length ).apply( intToString ).apply( 234 ) );
        assertTrue( 3 == this.<Integer, String, Integer>AND_THEN().apply( intToString ).apply( length ).apply( 234 ) );
